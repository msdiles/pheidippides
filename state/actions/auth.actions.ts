import {
  ...
  AuthLoginStartPayload,
  AuthRefreshDonePayload,
  AuthRefreshStartPayload,
...
export const authLoginStart = (
  ...
  RootState,
  unknown,
  Action<AuthActionsTypes | AppActionsTypes>
> => {
  return async (dispatch) => {
    try {
      await dispatch(authLoading())
      const { result, status } = await API.signin(
        payload.email,
        payload.password,
        payload.fingerprint
      )
      ...
      await dispatch(
        authLoginDone({ user: result.user, accessToken: result.accessToken })
      )
      await Router.push("/")
      await dispatch(
        appSetMessage({
...
export const authLogoutStart = (): ThunkAction<
  void,
  RootState,
  unknown,
  Action<AuthActionsTypes | AppActionsTypes>
> => {
  return async (dispatch, getState) => {
    try {
      await dispatch(authLoading())
      const { result, status } = await API.logout(
        getState().auth.user.userToken
      )
      if (status > 200) {
...
> => {
  return async (dispatch) => {
    try {
      ...
      if (status === 401) {
        await dispatch(
          appSetMessage({ message: "Your session expired", type: "warning" })
        )
        await dispatch(authLogoutDone())
        await Router.push("/")
      } else if (status > 200 && status !== 401) {
        await dispatch(
          appSetMessage({ message: "Something went wrong", type: "error" })
        )
        await dispatch(authLogoutDone())
        await Router.push("/")
      } else {
